...
#       # now start receiving packets
    ...
    def parse_collision(self, data):
        if self.is_connected:
            ...
            self.angle = math.atan2(self.cx, self.cy)*180/math.pi   # Calculating the collision angle with y-axis as the reference
#             self.angle = self.angle + 90
            if math.fabs(self.angle) <= 90:
                self.theta = 90 - math.fabs(self.angle)
                self.head =  self.head - math.copysign(1, self.angle)*2*self.theta 
                
                if math.fabs(self.head) > 180:
                    self.head = self.head - math.copysign(1,self.head) * 360
                
                print '[cx:{0}],[cy:{1}],[an:{2}],[hd:{3}]'.format(self.cx, self.cy,self.angle, self.head, self)
                
                self.collipy = np.vstack([self.collipy, np.matrix([self.cx, self.cy, self.theta, self.head, self.angle, self.cspeed])])
                
                self.t = np.vstack([self.t,self.t[-1]+1])
                if self.head < 0:
                    self.hd = self.head + 360
                else:
                    self.hd = self.head

                self.robot.roll(80, int(self.hd), 1, False)
